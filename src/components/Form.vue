    ...
    <div v-if="house">
        <form @submit.prevent="submitForm" novalidate class="form">
         ...
         -->
            ...
            <button v-if="isEditMode" type="submit" style="margin-left:auto">Save</button>
            <button v-else :disabled="!isFormValid" type="submit"
                :style="{ 'background-color': isFormValid ? 'originalColor' : 'disabledColor', 'margin-left': 'auto' }">Post</button>
        </form>
    </div>
    <div v-else>
        <h1>Loading...</h1>
